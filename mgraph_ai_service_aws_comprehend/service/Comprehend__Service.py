...
class Comprehend__Service(Type_Safe):                                                                      # Main orchestrator for AWS Comprehend operations with caching support
    comprehend_detect : Comprehend__Detect                                                                 # OSBot-AWS Comprehend detect instance
    ...
    def detect_sentiment(self                                                                       ,      # Detect sentiment with optional caching
        ...
        if use_cache:                                                                                      # Check cache first
            cached_result = self.cache_service.get_sentiment(text, language_code)
            if cached_result:
                return cached_result

        result = self.comprehend_detect.detect_sentiment(text, language_code)                             # Call native OSBot-AWS method

        if use_cache:                                                                                      # Store in cache
