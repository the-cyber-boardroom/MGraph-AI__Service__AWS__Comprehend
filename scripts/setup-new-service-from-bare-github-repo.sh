...
git fetch template
git merge template/main --allow-unrelated-histories -m "Initial import from MGraph-AI__Service__AWS__Comprehend template"
print_success "Template merged"

# Step 5: Rename the service directory
print_step "Renaming service directory from mgraph_ai_service_aws__comprehend to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_aws__comprehend" ]; then
    mv mgraph_ai_service_aws__comprehend "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_aws__comprehend directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service AWS Comprehend" with the new service title
replace_in_files "MGraph AI Service AWS Comprehend" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_aws__comprehend" with the new package name
replace_in_files "mgraph_ai_service_aws__comprehend" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__AWS__Comprehend" with the new repository name
replace_in_files "MGraph-AI__Service__AWS__Comprehend" "$REPO_NAME" "repository name"
...
- Based on MGraph-AI__Service__AWS__Comprehend v0.3.0
